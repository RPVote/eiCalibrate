...
#' raw_fit <- eiPack::ei.MD.bayes(fml, data = dat, total = "total",
#'                                lambda1 = 4, lambda2 = 2,
...
extract_rxc_estimates <- function(fit, row_names, col_names) {
  ...
  if (is.null(fit$draws) || is.null(fit$draws$Cell.counts)) {
    stop("Fit object does not contain $draws$Cell.counts. ",
  ...
  for (r in seq_along(row_names)) {
    for (c in seq_along(col_names)) {
      ...
      if (length(matched) == 0) {
        warning("No MCMC columns matched for row '", row_names[r],
                "' x col '", col_names[c],
...
iter_to_matrix <- function(iter_fit, row_names, col_names) {
  ...
  if (is.null(est_df)) {
    stop("Could not locate estimates table in ei_iter output. ",
  ...
  if (all(cand_pct_cols %in% colnames(est_df)) ||
    ...
    target_cols <- if (all(cand_pct_cols %in% colnames(est_df)))
                     cand_pct_cols else col_names
    # Find a column identifying race
    race_col <- intersect(c("race", "group", "Race", "Group"),
                          colnames(est_df))[1]
    if (!is.na(race_col)) {
      for (r in seq_along(row_names)) {
  ...
  if (all(is.na(mat))) {
    warning("Could not parse ei_iter estimates table. ",
